...
for example in raw_test_dataset:
    ...
    true_tags = [label_list[tag] for tag in example['ner_tags']]  # 原始标注
    ...
    if is_CRF:
        pred_tags_ids = outputs  # 取出预测的序列（假设返回的是 list of list）
    else:
        logits = outputs
    ...
    for idx, word_idx in enumerate(word_ids):
        ...
        if word_idx != previous_word_idx:
            pred_tags.append(label_list[pred_tags_ids[idx]])
            previous_word_idx = word_idx
            word = example["tokens"][word_idx]
            NER_tag = label_list[pred_tags_ids[idx]]
        
            print(f"{word:<15}{NER_tag:<15}")
